...
// routing middleWare that allows the routes defined on this file to be used on 
const router = express.Router();

router.use((req,res,next)=>{
    // req.session
    // check to see if the user is logged in via req.session.loggedIn property. This property was defined in the controller.user.js file
    // if the user is loggedIn we are going to use the next() which means allow the user to access the routes defined below
    if(req.session.loggedIn){
        next();
    }else{
        //Else the user is NOT loggedIn, therefore just have the user redirected
